...
func check_mouth_open(l : String) -> void:
	if 'AEIOUYaeiouy1234567890'.contains(l):
		open_mouth = true
		mouth_open_frames += 3
		audio_stream_player.pitch_scale = randf_range(audio_pitch_base - 0.4,audio_pitch_base + 0.4)
...
func blinker() -> void:
	...
	blink = !blink
	blinker() #function continues to call itself
	pass
